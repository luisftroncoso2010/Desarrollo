...
Estos obejetos debenimpmentar métodos espaciales '__enter__' 
y '__exist__'''
...
class GestorContextoEjemplo:
    def __enter__(self):
        ...
        #Puedes devolver cualquier objeto que quieras utilizar dentro de bloque with
        return self
    
    #exc_type: Este parámetro contiene el tipo de la excepción que se ha producido.
    #exc_value: Contiene la instancia de la excepción que se ha producido nuevamente,
...
class GestorContextNombre:
    def __enter__(self):
        #Se puede devolver cualquier objeto que queiras utilizar dentro de bloque with
        return self
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        if exc_type is not None:
            #sEprodijo una excepción, puede acceder a los parámetros
...
with open('Fichero.txt', 'w') as fichero:
    #La variable fichero no sera accedida desde afuera, una mentes desde el bloque 
    #with
    fichero.write('Hola')
...
class MiClaseFichero:
    #Creamos el contructor que resibirá el gestor y el fichero que queremos crear
